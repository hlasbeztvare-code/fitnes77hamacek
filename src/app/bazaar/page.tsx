...
export default async function BazaarPage() {
  const bazaarItems = await getAllBazaarItems();
          ...
          {bazaarItems.map((item) => (
            <article key={item.id} className="group relative flex flex-col overflow-hidden bg-white p-4 transition-all duration-500 border-r border-b border-zinc-100 rounded-none not-italic">
              <Link href={`/bazaar/${item.slug}`} className="relative z-10 block">
                <div className="relative aspect-[4/5] overflow-hidden bg-white rounded-none p-6">
                  {/* Štítek stavu */}
                  <div className="absolute left-0 top-0 z-30 bg-[#FF8C00] px-3 py-1 text-[9px] font-black uppercase tracking-widest text-black rounded-none">
                    {item.condition}
                  </div>
