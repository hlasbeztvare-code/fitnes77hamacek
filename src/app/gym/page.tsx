...
export default function GymPage() {
  ...
  const trainers = [
    {
      ...
      bio: "Zkušený powerlifter s mnohaletou praxí. Soutěžil na národní úrovni, trénoval řadu úspěšných závodníků."
    },
    {
      name: "Soustružník",
      role: "Trenér silového tréninku & kondice",
      img: "https://picsum.photos/id/1005/600/600",
      bio: "Specialista na funkční silový trénink a hypertrofii. Má za sebou mnoho let praxe."
    },
  ];

  const galleryImages = [
    { id: 1, src: "https://picsum.photos/id/1015/800/600", alt: "Gym 1" },
    { id: 2, src: "https://picsum.photos/id/201/800/600", alt: "Gym 2" },
    { id: 3, src: "https://picsum.photos/id/301/800/600", alt: "Gym 3" },
    { id: 4, src: "https://picsum.photos/id/401/800/600", alt: "Gym 4" },
  ];
  ...
  return (
    <div className="bg-black text-white overflow-x-hidden font-sans">
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {services.map((service, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} className="bg-zinc-900 p-8 border border-zinc-800 transition-all group">
                <div className="text-[rgb(222,255,95)] mb-6 text-5xl group-hover:scale-110 transition-transform">{service.icon}</div>
                <h3 className="text-2xl font-bold mb-4 uppercase italic">{service.title}</h3>
      ...
      {/* FOOTER - TVŮJ NEDOTKNUTELNEJ ORIGINÁL */}
      ...
      <AnimatePresence>
        {selectedImage !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedImage(null)} className="fixed inset-0 bg-black/95 z-[99999] flex items-center justify-center p-8">
            <img src={galleryImages[selectedImage].src} alt="Gallery" className="max-h-[90vh] max-w-[90vw] object-contain" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
