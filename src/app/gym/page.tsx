...
export default function GymPage() {
  ...
  return (
    <main className="bg-[#050505] text-white selection:bg-[#FF0000] selection:text-white overflow-x-hidden relative scroll-smooth min-h-screen font-black uppercase italic tracking-tighter">
      ...
      {/* 1. HERO - ESHOP FONT, VŠE ZMENŠENO (smrk) */}
      <section className="relative min-h-[40vh] flex flex-col justify-center px-4 md:px-6 border-b border-[#FF0000]/20 z-20 py-12">
        <Reveal y={20}>
          <div className="max-w-[1400px] mx-auto w-full relative z-10">
            <h1 className="text-[12vw] md:text-[8vw] leading-[0.7] text-white drop-shadow-[0_15px_30px_rgba(0,0,0,1)]">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-end mt-6 gap-6">
               <div className="flex flex-col gap-4 relative">
                 <p className="text-[7px] md:text-[8px] tracking-[0.8em] text-zinc-600">MLADÁ BOLESLAV // UNIT_01</p>
                 <div className="flex flex-col sm:flex-row gap-8 border-l-[6px] border-[#FF0000] pl-6 bg-black/80 backdrop-blur-xl p-5 shadow-2xl border border-white/5">
                    <div className="absolute inset-y-0 -left-1.5 w-1.5 bg-[#FF0000]"></div>
                    <div>
                      <p className="text-[9px] text-zinc-500 tracking-widest uppercase mb-1">PO - PÁ</p>
                    ...
                    <div className="sm:border-l sm:border-white/10 sm:pl-10">
                      <p className="text-[9px] text-zinc-500 tracking-widest uppercase mb-1">SO - NE</p>
                      <p className="text-xl md:text-2xl text-white tracking-tighter font-black italic">09:00 - 20:00</p>
                    </div>
                 </div>
               </div>
               <Link href="/shop" onClick={() => triggerHaptic(30)} className="px-10 py-4 bg-white text-black text-[10px] tracking-[0.2em] hover:bg-[#FF0000] hover:text-white transition-all shadow-2xl text-center font-black italic">VSTOUPIT DO E-SHOPU</Link>
            </div>
          </div>
        </Reveal>
      </section>

      {/* 2. STATS - ESHOP FONT (smrk) */}
          ...
          {[
            ...
            <Link href={item.l} key={idx} onClick={() => triggerHaptic(15)} className="relative h-64 overflow-hidden group border-2 border-white/5">
              ...
              <div className="absolute bottom-5 left-5">
                <p className="text-3xl text-[#FF0000] tracking-tighter drop-shadow-lg leading-none">{item.t}</p>
                <p className="text-[9px] font-bold text-white tracking-[0.2em] uppercase leading-tight mt-1 italic">{item.d}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* 3. TRENÉŘI - ESHOP FONT, VŠE ZMENŠENO (smrk) */}
        ...
        <div className="max-w-[1700px] mx-auto flex flex-col lg:grid lg:grid-cols-3 gap-24 md:gap-16 items-start relative z-10">
          ...
          <div className="w-full flex flex-col group relative order-2 lg:order-1 scale-95 origin-top">
            ...
            <div className="relative -mt-24 mx-4 bg-black/90 backdrop-blur-3xl p-8 border-2 border-white/10 shadow-2xl z-20">
              ...
              <p className="text-[10px] text-zinc-500 uppercase mt-2 border-l-2 border-[#FF0000] pl-4 font-black italic">HLAVNÍ KOUČ</p>
              <div className="mt-6 pt-6 border-t border-white/5">
                <p className="text-[9px] text-[#FF0000] tracking-widest mb-3 uppercase italic font-black">DOPORUČENÉ SUPLE</p>
          ...
          {/* STŘED: SLUŽBY & CENÍK - ESHOP FONT (smrk) */}
          <div className="w-full flex flex-col gap-10 order-1 lg:order-2 text-center relative z-20">
              ...
              <div className="space-y-3 relative z-10 px-2 flex flex-col items-center">
                {sluzby.map((s, i) => (
                  <motion.div key={i} whileTap={{ scale: 0.98 }} className="flex items-center justify-center gap-4 group border border-zinc-800 py-2.5 px-6 hover:border-[#FF0000] transition-colors bg-white/5 w-full">
                    ...
                    <p className="tracking-tighter text-xs md:text-sm text-zinc-300 group-hover:text-white uppercase leading-none font-black italic">
                      {s.n}
                    </p>
                  </motion.div>
                ))}
            ...
            <div className="flex flex-col group relative text-center">
              ...
              <div className="relative -mt-8 mx-1 bg-black/80 backdrop-blur-3xl p-6 border-2 border-white/10 shadow-2xl z-20">
                ...
                <Link href="/shop" onClick={() => triggerHaptic(30)} className="block w-full text-center py-4 bg-white text-black text-[9px] hover:bg-[#FF0000] hover:text-white transition-all shadow-2xl relative z-10 tracking-[0.3em] mt-6 font-black italic">VSTOUPIT DO E-SHOPU →</Link>
              </div>
            </div>
          </div>
