...
export default function GymPage() {
  ...
  return (
    <main className="bg-[#050505] text-white selection:bg-[#FF0000] selection:text-white overflow-x-hidden relative scroll-smooth min-h-screen font-black uppercase tracking-tighter">
      ...
      {/* 1. HERO - PŘESNĚ TEN ROVNEJ FONT (smrk) */}
          ...
          <div className="max-w-[1400px] mx-auto w-full relative z-10">
            ...
            <div className="flex flex-col md:flex-row justify-between md:items-end mt-12 gap-10">
               <div className="flex flex-col gap-6 relative">
                 ...
                 <div className="flex flex-col sm:flex-row gap-10 border-l-[6px] border-[#FF0000] pl-6 bg-black/80 backdrop-blur-xl p-8 shadow-2xl border border-white/5">
                    <div>
               ...
               <Link href="/shop" className="px-12 py-5 bg-white text-black font-black uppercase text-[11px] tracking-[0.3em] hover:bg-[#FF0000] hover:text-white transition-all shadow-2xl text-center">
                 VSTOUPIT DO E-SHOPU →
               </Link>
            </div>
          </div>
      ...
      {/* 3. CORE CONTENT - HAMÁČEK & SOUSTRUŽNÍK (smrk) */}
      <section className="relative py-20 px-4 z-10">
        <div className="max-w-[1700px] mx-auto flex flex-col lg:grid lg:grid-cols-3 gap-16 items-start relative z-10">
          
          {/* HAMÁČEK */}
          <div className="w-full flex flex-col group relative order-2 lg:order-1 scale-95 origin-top font-black">
            ...
            <div className="relative -mt-24 mx-4 bg-black/90 backdrop-blur-3xl p-8 border-2 border-white/10 shadow-2xl z-20">
              ...
              <h3 className="text-4xl uppercase tracking-tighter text-white font-black">HAMÁČEK</h3>
              <p className="text-[10px] font-black text-zinc-500 uppercase mt-2 border-l-2 border-[#FF0000] pl-3">HLAVNÍ KOUČ</p>
              <Link href="/kontakt" className="block w-full text-center py-5 bg-[#FF0000] text-white text-[10px] uppercase mt-8 tracking-[0.2em] shadow-xl font-black">REZERVOVAT</Link>
            </div>
          </div>

          {/* SLUŽBY & CENÍK */}
          <div className="w-full flex flex-col gap-12 order-1 lg:order-2 text-center relative z-20 font-black">
            <div className="relative bg-black/80 backdrop-blur-3xl p-8 border-2 border-white/10 shadow-2xl">
              ...
              <div className="space-y-3 px-2 flex flex-col items-center">
                {sluzby.map((s, i) => (
                  <div key={i} className="flex items-center justify-center gap-4 border border-zinc-800 py-2.5 px-6 hover:border-[#FF0000] transition-colors bg-white/5 w-full">
                    <span className="text-xl grayscale group-hover:grayscale-0">{s.i}</span>
                    <p className="tracking-tighter text-xs font-black text-zinc-300 uppercase">{s.n}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="relative bg-black/80 backdrop-blur-3xl p-6 border-2 border-white/10 shadow-2xl">
              <div className="grid grid-cols-3 gap-2 font-black">
                {[{ n: 'JEDNORÁZ', p: '160' }, { n: 'MĚSÍČNÍ', p: '1490' }, { n: 'ROČNÍ', p: '12990' }].map((item, idx) => (
                  <div key={idx} className="flex flex-col justify-center items-center py-6 border border-white/5">
                    <h4 className="text-[10px] tracking-tighter text-zinc-500 uppercase mb-1 leading-none">{item.n}</h4>
                    <span className="text-lg text-[#FF0000] font-black">{item.p} CZK</span>
                  </div>
                ))}
              </div>
              <Link href="/shop" className="block w-full text-center py-4 bg-white text-black text-[9px] hover:bg-[#FF0000] hover:text-white transition-all shadow-2xl mt-6 font-black uppercase tracking-[0.3em]">VSTOUPIT DO E-SHOPU →</Link>
            </div>
          </div>

          {/* SOUSTRUŽNÍK */}
          <div className="w-full flex flex-col group relative order-3 text-right scale-95 origin-top font-black">
            <div className="relative w-full aspect-[4/5] bg-zinc-900 overflow-hidden shadow-2xl border border-white/5">
            ...
            <div className="relative -mt-24 mx-4 bg-black/90 backdrop-blur-3xl p-8 border-2 border-white/10 shadow-2xl z-20 flex flex-col items-end">
              ...
              <h3 className="text-4xl font-black uppercase tracking-tighter text-white">SOUSTRUŽNÍK</h3>
              <p className="text-[10px] font-black text-zinc-500 uppercase mt-2 border-r-2 border-[#FF0000] pr-4">PRO UNIT</p>
              <Link href="/kontakt" className="block w-full text-center py-5 bg-[#FF0000] text-white text-[10px] font-black uppercase mt-8 tracking-[0.2em] shadow-xl">REZERVOVAT</Link>
            </div>
          </div>
        </div>
      </section>
