...
export default function GymPage() {
  ...
  return (
    <main className="min-h-screen selection:bg-[#E10600] selection:text-white overflow-x-hidden scroll-smooth bg-white">
      ...
      {/* 1. HERO - ZMENŠENO A ZHUŠTĚNO (smrk) */}
      <section className="relative h-[35vh] flex flex-col justify-center px-6 bg-white z-20 border-b border-zinc-100">
      ...
      {/* 3. ZBYTEK - ŠEDÉ SKLO A KOMPAKTNÍ BOX (smrk) */}
      <section className="relative py-16 px-4 z-10 bg-[#E5E7EB] bg-opacity-20 backdrop-blur-3xl min-h-[60vh]">
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="group relative lg:mt-10">
            <div className="relative bg-white/70 backdrop-blur-xl p-8 border border-white/40 shadow-xl">
          ...
          <div className="group relative text-right">
            ...
            <div className="relative -mt-16 mx-8 bg-white/70 backdrop-blur-xl p-5 border border-white/40 shadow-xl flex flex-col items-end">
              ...
              <div className="mt-3 p-2 bg-black/5 border border-black/5 rounded w-full">
                <p className="text-[9px] font-black text-zinc-400 text-right">STACK: VITAMINS + ZMA</p>
              </div>
            </div>
          </div>
        </div>

        {/* GALERIE - KOMPAKTNÍ (smrk) */}
        <div className="mt-24 grid grid-cols-4 md:grid-cols-8 lg:grid-cols-12 gap-1 px-4 max-w-[1400px] mx-auto">
          {galleryFiles.map((file, i) => (
            <div key={i} className="relative aspect-square cursor-pointer overflow-hidden opacity-70 hover:opacity-100 transition-all" onClick={() => setSelectedImage(`/images/gym/gallery/${file}`)}>
              <img src={`/images/gym/gallery/${file}`} className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
      </section>
