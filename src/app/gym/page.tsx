...
export default function GymPage() {
  ...
  return (
    <main className="bg-black text-white overflow-x-hidden relative scroll-smooth min-h-screen font-black uppercase tracking-tighter">
      ...
      {/* 1. HERO - BETON JE JENOM TADY (smrk) */}
      <section className="relative min-h-[85vh] flex flex-col justify-center px-6 z-20 py-24 overflow-hidden border-b border-[#FF0000]/20">
        ...
        <Reveal y={20}>
          <div className="max-w-[1400px] mx-auto w-full relative z-10">
            <h1 className="text-[16vw] md:text-[12vw] leading-[0.7] text-white font-black uppercase drop-shadow-[0_10px_30px_rgba(0,0,0,1)]">
              FITNESS<span className="text-[#FF0000]">77</span>
            </h1>
            <div className="mt-12 bg-black/90 p-8 shadow-2xl inline-block border-l-[8px] border-[#FF0000] backdrop-blur-md">
              <p className="text-[10px] text-zinc-500 font-bold tracking-[0.4em] mb-2 uppercase">MLADÁ BOLESLAV // ORIGINÁL</p>
      ...
      <section className="relative py-16 px-4 z-20 bg-black">
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div key={idx} className="relative h-64 overflow-hidden border border-white/10 group">
              <Image src={item.i} alt={item.t} fill className="object-cover grayscale opacity-20 group-hover:opacity-50 transition-all duration-700" />
              <div className="absolute bottom-6 left-6">
                <p className="text-4xl text-[#FF0000] font-black leading-none">{item.t}</p>
                <p className="text-[11px] font-black text-white tracking-widest mt-2">{item.d}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* 3. TRENÉŘI, SLUŽBY A CENÍK - VŠE POHROMADĚ (smrk) */}
        ...
        <div className="max-w-[1700px] mx-auto flex flex-col lg:grid lg:grid-cols-3 gap-16 items-start relative z-10">
          ...
          <div className="w-full flex flex-col font-black">
            ...
            <div className="relative -mt-20 mx-4 bg-black p-8 border-t-4 border-[#FF0000] shadow-2xl">
              <h3 className="text-5xl tracking-tighter font-black uppercase">HAMÁČEK</h3>
              <p className="text-[11px] text-zinc-500 tracking-[0.4em] mb-4 uppercase">HLAVNÍ KOUČ</p>
              <Link href="/kontakt" className="block w-full text-center py-5 bg-[#FF0000] text-white text-[11px] font-black tracking-[0.3em] hover:bg-white hover:text-black transition-all">REZERVOVAT</Link>
            </div>
          </div>
          ...
          <div className="w-full flex flex-col gap-10 order-first lg:order-none font-black text-center">
            <div className="relative bg-zinc-900/50 p-8 border border-white/5 shadow-2xl">
            ...
            <div className="bg-zinc-900/50 p-8 border border-white/5 shadow-2xl">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[{ n: 'JEDNORÁZ', p: '160' }, { n: 'MĚSÍČNÍ', p: '1490' }, { n: 'ROČNÍ', p: '12990' }].map((item, idx) => (
                  <div key={idx} className="flex flex-col items-center py-6 border border-white/5 bg-black/40">
                    <h4 className="text-[9px] text-zinc-500 mb-2 font-black uppercase tracking-widest">{item.n}</h4>
                    <span className="text-xl text-[#FF0000] font-black">{item.p} CZK</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="w-full flex flex-col font-black text-right">
            ...
            <div className="relative -mt-20 mx-4 bg-black p-8 border-t-4 border-[#FF0000] shadow-2xl flex flex-col items-end">
              <h3 className="text-5xl tracking-tighter font-black uppercase">SOUSTRUŽNÍK</h3>
