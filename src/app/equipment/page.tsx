      ...
      <div className="mx-auto w-[min(1280px,calc(100%-32px))]">
        ...
        <div className="mt-12 grid grid-cols-2 gap-4 lg:grid-cols-4 sm:gap-6">
          {items.map((item) => {
            ...
            const cardImage = item.name.toLowerCase().includes('opasek') ? '/videos/pasek.webm' : item.image;
            const isVideo = cardImage.toLowerCase().match(/.(mp4|webm)$/i);
            ...
            return (
              <article key={item.id} className="group relative flex flex-col overflow-hidden bg-white p-4 transition-all duration-500 hover:z-10 rounded-none not-italic">
                ...
                <Link href={`/equipment/${item.slug}`} className="relative z-10 block">
                  <div className="relative aspect-[4/5] overflow-hidden bg-white transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:shadow-lg rounded-none p-6 transition-transform duration-500 group-hover:-translate-y-2 flex items-center justify-center">
