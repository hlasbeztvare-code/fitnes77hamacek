...
export const Services = () => {
  return (
    <section id="sluzby" className="bg-[#000000] py-20 md:py-40">
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 text-center">
        <h2 className="text-[16vw] md:text-[12vw] font-black font-black leading-none mb-12 md:mb-32 tracking-tighter uppercase">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-10">
          {services.map((s, i) => (
            <Link key={i} href={s.href}>
              <motion.div
                whileHover={{ y: -10 }}
                className="bg-black/50 p-5 md:p-10 rounded-2xl md:rounded-3xl border border-white/5 hover:border-[#d4ff00]/50 transition-all text-left group cursor-pointer h-full flex flex-col"
              >
                <div className="mb-3 md:mb-6 [&>svg]:w-6 [&>svg]:h-6 md:[&>svg]:w-10 md:[&>svg]:h-10">{s.icon}</div>
                <h3 className="text-lg md:text-3xl font-black font-bold mb-2 md:mb-4 group-hover:text-[#d4ff00] transition-colors leading-tight">{s.title}</h3>
                <p className="text-white/40 font-medium uppercase text-[9px] md:text-sm leading-relaxed flex-1 hidden md:block">{s.desc}</p>
