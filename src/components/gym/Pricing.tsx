...
const Pricing = () => {
  ...
  return (
    <section id="pricing" className="bg-[#050505] text-white py-20 md:py-40 px-4 md:px-32 relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        <div className="flex flex-col mb-20">
           ...
           <h2 className="text-[16vw] md:text-[12vw] font-black leading-[0.8] tracking-tighter uppercase font-bebas mb-3 md:mb-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {plans.map((plan, i) => {
            ...
            <div
              key={i}
              className={`relative bg-[#050505] p-4 md:p-6 lg:p-8 flex flex-col justify-between h-[280px] md:h-[380px] lg:h-[450px] transition-all duration-500 border ${
                isAnnual 
            ...
            >
              ...
              <div>
                <span className={`text-[9px] md:text-xs font-bold uppercase tracking-[0.4em] block mb-3 md:mb-6 font-space ${isAnnual ? 'text-[#d4ff00]' : 'text-white/40'}`}>Option 0{i+1}</span>
                <h3 className="text-xl md:text-3xl font-black leading-tight uppercase mb-2 md:mb-4 font-bebas tracking-wider whitespace-pre-line break-words">{plan.title}</h3>
                <p className="text-[9px] md:text-xs font-bold opacity-50 uppercase tracking-widest font-space whitespace-pre-line">{plan.sub}</p>
              </div>
              <div>
                <div className={`text-3xl md:text-5xl font-black mb-4 md:mb-8 tracking-tighter font-bebas ${isAnnual ? 'text-[#d4ff00]' : ''}`}>{plan.priceStr}</div>
                <button
                  onClick={() => addItem({
                    id: plan.id,
                    ...
                    slug: plan.id,
                    price: plan.priceNum,
                    image: '/images/brand/logo-fitness77.png',
                  })}
                  className={`w-full py-3 md:py-4 border font-black uppercase font-bebas tracking-widest text-base md:text-xl transition-all duration-500 hover:scale-105 active:scale-95 ${
                    isAnnual 
            ...
            <div className="space-y-16">
                ...
                <div className="space-y-16 mt-20">
                    ...
                    <Link href="/gym/jaroslav-hamacek" className="block group cursor-pointer">
                        ...
                        <p className="text-xl font-medium text-white/40 uppercase tracking-[0.2em] font-space max-w-sm group-hover:text-white/80 transition-colors mb-4">MAJITEL / BOX / KOMUNITA</p>
                        <div className="flex items-center gap-6 flex-wrap">
                    ...
                    <Link href="/gym/beata-cejnarova" className="block group cursor-pointer">
                        ...
                        <p className="text-xl font-medium text-white/40 uppercase tracking-[0.2em] font-space max-w-sm group-hover:text-white/80 transition-colors mb-4">FITNESS TRENÉRKA / NUTRIČNÍ KOUČKA</p>
                        <div className="flex items-center gap-6 flex-wrap">
