...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-[100] bg-white border-b border-zinc-100 px-6 py-4 flex justify-between items-center font-black uppercase tracking-tighter text-[11px]">
      <Link href="/" className="text-xl tracking-tighter font-black">FITNESS<span className="text-[#E10600]">77</span></Link>
      <div className="hidden md:flex gap-10">
        {navLinks.map((link) => (
          ...
          >
            {link.name}
            {/* TOHLE JE TO PODTRŽENÍ, CO JSEM TI VRÁTIL (smrk) */}
            {pathname === link.href && (
              <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-[#E10600]"></div>
            )}
          </Link>
        ))}
      </div>
