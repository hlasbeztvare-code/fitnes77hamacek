      ...
      <AnimatePresence>
        {mobileOpen && (
          ...
          >
            ...
            {/* Košík odkaz v mobilním menu */}
            <Link
              href="/cart"
              onClick={() => setMobileOpen(false)}
              className="mt-6 flex items-center gap-3 border border-white/20 px-8 py-4 text-white font-bold uppercase tracking-widest hover:border-[#E10600] hover:text-[#E10600] transition-all"
            >
              <ShoppingCart className="h-5 w-5" />
              Košík
              {mounted && totalItems > 0 && (
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[#E10600] text-xs font-black text-white">
                  {totalItems}
                </span>
              )}
            </Link>
