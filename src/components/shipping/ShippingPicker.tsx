    ...
    <div className="bg-zinc-50 border border-zinc-200 p-5 lg:p-6 min-w-0">
      ...
      <div className="space-y-3">
        {Object.values(SHIPPING_METHODS).map((method) => (
          <label 
            key={method.id} 
            className={`flex items-start p-4 border cursor-pointer transition-all ${selectedShipping === method.id ? 'border-[#E10600] bg-[#E10600]/5' : 'border-zinc-200 bg-white hover:border-zinc-400'}`}
          >
            ...
            <div className="ml-4 flex-1 min-w-0">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-black text-black uppercase tracking-wider">{method.name}</span>
              ...
              {method.id === 'ZASILKOVNA' && selectedShipping === 'ZASILKOVNA' && (
                <div className="mt-3 bg-zinc-50 border border-zinc-200 p-3 flex justify-between items-center">
                  <span className="text-sm text-black font-bold">{shippingPointName || 'Nevybráno (Zásilkovna / AlzaBox)'}</span>
                  <button 
                    type="button" 
                    onClick={(e) => { e.preventDefault(); handlePacketaOpen(); }}
                    className="text-xs font-black uppercase tracking-widest text-[#E10600] hover:text-black transition-colors"
                  >
